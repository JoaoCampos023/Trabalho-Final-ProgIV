...
(function () {
  ...
  interface DashboardAppState {
    ...
    sortCampoAnimais: string;
    sortDirAnimais: SortDir;
    ultimoResultadoAnimais: {
      animais: Animal[];
      stats: any;
      racas: string[];
      searchNome: string;
    ...
    ordenarAnimaisPor(campo: string): void;
    renderAnimais(): void;
    loadAnimais(): Promise<void>;

    sortCampoProducoes: string;
    sortDirProducoes: SortDir;
    ultimoResultadoProducoes: ProducaoLeite[] | null;
    ...
    sortCampoUsuarios: string;
    sortDirUsuarios: SortDir;
    ultimoResultadoUsuarios: Usuario[] | null;
    ordenarUsuariosPor(campo: string): void;
    renderUsuarios(): void;
  ...
  const app: DashboardAppState = {
    ...
    renderAnimais() {
      const resultado: DashboardAppState['ultimoResultadoAnimais'] = this.ultimoResultadoAnimais;
      if (!resultado) return;
      ...
      if (!animais || animais.length === 0) {
        ...
        const th = (label: string, campo: string) =>
          Components.thOrdenavel(
            label,
            campo,
            this.sortCampoAnimais,
            this.sortDirAnimais,
            `app.ordenarAnimaisPor('${campo}')`
          );
    ...
    renderProducoes() {
      const resultado: ProducaoLeite[] | null = this.ultimoResultadoProducoes;
      if (!resultado) return;
      ...
      const producoes = Components.ordenarLista(
        resultado,
        this.sortCampoProducoes,
        this.sortDirProducoes,
        (p, campo) => (campo === 'animal' ? p.animal?.nome || p.animal_brinco : (p as any)[campo])
      );
      const th = (label: string, campo: string) =>
        Components.thOrdenavel(
          label,
    ...
    async loadRelatorios() {
      ...
      try {
        ...
        const producaoData: { producoes?: ProducaoLeite[]; stats?: { totalLitros: number } } =
          producaoRes.data?.data || {};
    ...
    renderUsuarios() {
      const resultado: Usuario[] | null = this.ultimoResultadoUsuarios;
      if (!resultado) return;
      ...
      const th = (label: string, campo: string) =>
        Components.thOrdenavel(
          label,
          campo,
          this.sortCampoUsuarios,
          this.sortDirUsuarios,
          `app.ordenarUsuariosPor('${campo}')`
        );
