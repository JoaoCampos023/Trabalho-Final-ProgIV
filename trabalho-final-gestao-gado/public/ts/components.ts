...
(window as any).logoutGlobal = logoutGlobal;
...
const Components = {
  ...
  renderNavbar(activePage: string, user: UsuarioLogado | null, mode?: NavMode): void {
    ...
    // ---- Menu do usuário ----
    const menu = container.querySelector<HTMLElement>('#userMenu');
    const trigger = container.querySelector<HTMLButtonElement>('#userMenuTrigger');
    const dropdown = container.querySelector<HTMLElement>('#userMenuDropdown');
