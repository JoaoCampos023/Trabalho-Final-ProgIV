...
// ✅ Função auxiliar para converter tipos do banco
function parseAnimal(row: any): Animal {
    return new Animal({
        brinco: parseInt(row.brinco),
        nome: row.nome,
        sexo: row.sexo,
        raca: row.raca,
        peso: parseFloat(row.peso) || 0,
        data_nascimento: new Date(row.data_nascimento),
        ativo: row.ativo === true || row.ativo === 't' || row.ativo === 1,
        brinco_pai: row.brinco_pai ? parseInt(row.brinco_pai) : undefined,
        brinco_mae: row.brinco_mae ? parseInt(row.brinco_mae) : undefined,
...
export class AnimalRepository {
    ...
    async findAll(): Promise<Animal[]> {
        ...
        return result.rows.map(row => parseAnimal(row));
    }
    ...
    async findByBrinco(brinco: number): Promise<Animal | null> {
        ...
        return parseAnimal(result.rows[0]);
    }
    ...
    async findWithParents(brinco: number): Promise<Animal | null> {
        ...
        const animal = parseAnimal(row);

        // Adicionar pai e mãe se existirem
if (row.pai_brinco) {
    animal.pai = new Animal({
        brinco: parseInt(row.pai_brinco),
        nome: row.pai_nome,
        sexo: row.pai_sexo,
        peso: 0,
...
if (row.mae_brinco) {
    animal.mae = new Animal({
        brinco: parseInt(row.mae_brinco),
        nome: row.mae_nome,
        sexo: row.mae_sexo,
        peso: 0,
        data_nascimento: new Date(),
    ...
    async create(animalData: Omit<IAnimal, 'created_at' | 'updated_at'>): Promise<Animal> {
        // Validações de linhagem (mantidas)
